...
2. `fnc_applyCamoAction` is the scheme action's `statement`: replicates the dialog's 3-step progressive delay (`CBA_fnc_waitAndExecute`, no dialog UI involved) before calling `cfr_common`'s `fnc_setCamo`.
...
| `fnc_applyCamoAction` | `[schemeId]` | ACE self-action statement: the dialog-free 3-step apply sequence, then `cfr_common`'s `fnc_setCamo` |
