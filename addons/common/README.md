...
| `fnc_washOff` | `[unit]` | Washes camo off a swimming or diving unit if **Will Camo Wash Off in Water** is on; called every 3 seconds on the local player |
...
A second CBA setting, **Will Camo Wash Off in Water** (checkbox, default on, `isGlobal` 1), makes a player lose their camo when they swim or dive - within a few seconds of entering the water, since the check only runs every `WASH_OFF_CHECK_SECONDS`, which doubles as a short grace period for a quick dip: the face goes back to the base face, through `fnc_unsetCamo`, so the wear-off timer is cancelled, `cfr_common_camoRemoved` is raised and the usual "camouflage removed" hint is shown. `XEH_postInit.sqf` runs a `CBA_fnc_addPerFrameHandler` every `WASH_OFF_CHECK_SECONDS` (3, in `script_component.hpp`) on clients with an interface that calls `fnc_washOff` for the local `player`; it reads `ace_common_fnc_isSwimming` (the surface-swim and dive animation sets, so wading and vehicles never count) and returns before doing anything else while the player isn't swimming. It checks the unit's face with `fnc_isCamoFace` rather than the synced scheme variable, so camo restored from a loadout is washed off too. Only the local player is affected; AI and remote-controlled units keep their camo until it is cleaned, like wear-off. Painting can't be started while swimming in the first place: ACE's interaction menu and progress bars are disabled while swimming unless an action opts out, and none of this mod's do.
