...
| `fnc_useFacepaint` | `[unit, schemeId]` | Spends one use for that scheme: one round of a carried stick (through ACE's `adjustMagazineAmmo`), else a legacy plain item for free. Takes the round from the most-used stick when the scheme accepts several kinds. Returns the uses left for the scheme over every kind of stick it accepts (0 = now empty), -1 if there is nothing to report (a legacy item was used, or is still carried), -2 if no usable facepaint, -3 if the scheme doesn't exist. Called by `cfr_dialog` just before `fnc_setCamo`, which itself never spends anything |
...
| `fnc_handleAnimChanged` | `[unit, animation]` | `AnimChanged` handler on the local player - washes camo off when they dive, if **Will Camo Wash Off in Water** is on |
...
A second CBA setting, **Will Camo Wash Off in Water** (checkbox, default on, `isGlobal` 1), makes a player lose their camo the moment they dive: the face goes back to the base face, through `fnc_unsetCamo`, so the wear-off timer is cancelled, `cfr_common_camoRemoved` is raised and the usual "camouflage removed" hint is shown. There is no timer or polling. `XEH_postInit.sqf` uses `CBA_fnc_addPlayerEventHandler` (`"unit"`) to keep one `AnimChanged` handler (`fnc_handleAnimChanged`) on the local player's current unit only, moved across respawns and unit switches. The handler recognises diving by animation set, the same way ACE's `ace_common_fnc_isSwimming` reads swimming: the dive sets `Abdv`, `Asdv` and `Adve` are underwater, while the surface-swim sets `Aswm`, `Absw` and `Assw` are not - so swimming with your head above water leaves camo alone, and vehicles never match. It checks the unit's face with `fnc_isCamoFace` rather than the synced scheme variable, so camo restored from a loadout is washed off too. Only the local player is affected; AI and remote-controlled units keep their camo until it is cleaned, like wear-off. Painting can't be started while swimming in the first place: ACE's interaction menu and progress bars are disabled while swimming unless an action opts out, and none of this mod's do.
