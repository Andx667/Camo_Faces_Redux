 ...
 * least one scheme listed under it has a variant for the unit's current face - otherwise the pattern
 * list would come up empty, which is what a DLC-added face would otherwise hit under Vanilla (BI
 * authored its camo faces for the original heads only).
 *
 * Arguments:
 * 0: Unit <OBJECT>
 *
...

{
    ...
    if ((_itemClasses findIf {_x in _uniformItems}) != -1) then {
        private _offered = GVAR(schemes) findIf {
            _x params ["", "_pairs", "", "", "", "", "_categories"];
            (_categories findIf {_x == _categoryId}) != -1 && {(_pairs findIf {(_x select 0) == _face}) != -1}
        };
